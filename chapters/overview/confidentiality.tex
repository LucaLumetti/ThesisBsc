...
So, a function $f(\cdot)$ is $\mathsf{negligible}$ (typically denoted with $\mathsf{negl}$) if for every polynomial $p(\cdot)$ there exists an $N$ such that for all integers $n > N$ it holds that $f(n) < \frac{1}{p(n)}$.
    ...
    \end{itemize}
In both CPA and CCA, the adversary is able to obtain the encryption or the decryption of any message of his choice by using an \emph{oracle}, a black-box that outputs the requested operations over a message of his choice.
